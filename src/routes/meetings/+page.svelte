<script lang="ts">
  ...
  async function generateTitle() {
    ...
    try {
      const t = await meetingApi.qa(
        selectedMeeting.id,
        '请用一句话（20字以内）概括这个会议的主题，只输出标题，不要引号。',
      );
      ...
      if (newTitle) {
        ...
        meetings = meetings.map((x) =>
          x.id === selectedMeeting?.id ? { ...x, title: newTitle } : x,
        );
      }
    } catch (e) {
      console.error(e);
  ...
  async function saveTitle() {
    ...
    if (titleValue.trim() && selectedMeeting && titleValue !== selectedMeeting.title) {
      ...
      meetings = meetings.map((x) =>
        x.id === selectedMeeting?.id ? { ...x, title: titleValue.trim() } : x,
      );
    }
  }
  ...
  async function toggleRecording() {
    ...
    try {
      ...
      let segMap = new SvelteMap<number, { text: string; is_final: boolean }>();
  ...
  async function stopRecording() {
    ...
    if (selectedMeeting) {
      try {
        await asrApi.stopRecording(selectedMeeting.id);
        // 落库后刷新会议数据，合并实时转录
        const updated = await meetingApi.get(selectedMeeting.id);
        selectedMeeting = updated;
        meetings = meetings.map((x) => (x.id === updated.id ? updated : x));
    ...
    {#if selectedMeeting}
      ...
      <div class="detail-header">
            ...
            {#if m.transcript && !editingTitle}
              ...
              >
                <svg
                  ...
                  stroke="currentColor"
                  stroke-width="2"
                  ><path d="M12 3v18" /><path d="M5 7l7 7 7-7" /><path d="M5 17h14" /></svg
                >
              </button>
            {/if}
          </div>
        ...
        <div class="header-actions">
          ...
          <button
            class="ghost-btn"
            onclick={() => exportContent(m.summary || m.transcript, `${m.title}_导出`)}>导出</button
          >
        </div>
      </div>
      ...
      >
        <button
          ...
          tabindex={activeTab === 'summary' ? 0 : -1}>摘要</button
        >
        <button
          class="tab"
          ...
          tabindex={activeTab === 'transcript' ? 0 : -1}>转录</button
        >
      </div>
          ...
          {#if m.summary}
            <div class="content-head">
              ...
              <div class="head-actions">
                ...
                <button class="mini-btn" onclick={() => exportContent(m.summary, `${m.title}_摘要`)}
                  >导出</button
                >
              </div>
            </div>
          ...
          {#if m.transcript || liveTranscript || recording}
            <div class="content-head">
              ...
              <div class="head-actions">
                ...
                <button
                  class="mini-btn"
                  onclick={() => exportContent(m.transcript, `${m.title}_转录`)}>导出</button
                >
              </div>
            </div>
