<script lang="ts">
  ...
  // 模型配置
  type ModelOption = { id: string; display_name: string | null; model_id: string };
  ...
  onMount(async () => {
    try {
      models = await invoke<ModelOption[]>('model_list');
    ...
    <nav class="nav-list">
      ...
      <button class="nav-item" class:active={activeTab === 'ocr'} onclick={() => switchTab('ocr')}>
        <svg
          ...
          /><path d="M7 21H5a2 2 0 0 1-2-2v-2" /><line x1="7" x2="17" y1="10" y2="10" /><line
            x1="7"
            x2="17"
            y1="14"
            y2="14"
      ...
      <div class="toolbar">
        <div class="lang-bar">
          ...
          >
            <svg
              ...
              stroke="currentColor"
              stroke-width="2"
              ><path d="m7 16-4-4 4-4" /><path d="M3 12h17" /><path d="m17 8 4 4-4 4" /><path
                d="M21 12H4"
              /></svg
            >
          </button>
        ...
        <div class="toolbar-right">
          <select
            class="model-select"
            id="translate-model"
            bind:value={translateModelId}
            onchange={saveTranslateModel}
            aria-label="翻译模型"
          >
        ...
        <div class="builtin-section">
          ...
          {:else}
            <div class="builtin-list">
              {#each builtinGlossaries as g (g.file)}
