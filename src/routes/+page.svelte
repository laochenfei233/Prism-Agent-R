<script lang="ts">
  ...
  let providers = $state<unknown[]>([]);
  let models = $state<unknown[]>([]);

  async function load() {
    providers = await invoke<unknown[]>('model_providers');
    models = await invoke<unknown[]>('model_list');
  }
...
<div class="dashboard">
  <DashboardHeader
    agentCount={dashboardStore.overview?.agents.length ?? agentStore.agents.length}
  />
        ...
        <div class="setup-banner-content">
          <span class="setup-icon">
            <svg
              ...
              stroke-width="2"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z" /></svg
            >
          </span>
