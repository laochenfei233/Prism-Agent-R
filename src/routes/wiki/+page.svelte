<script lang="ts">
  ...
  async function loadEmbStatus() {
    try {
      ...
      providers = await invoke<{ id: string; name: string; kind: string }[]>('model_providers');
  ...
  async function toggleRerank() {
    try {
      await ragApi.rerankConfig(rerankEnabled);
    } catch (e) {
      console.error(e);
    }
  }
  ...
  async function openPage(path: string) {
    ...
    editing = false;
    try {
      pageContent = await wikiApi.readPage(selectedWiki.id, path);
    } catch (e) {
      console.error(e);
    }
  }
  ...
  async function handleSearch() {
    if (!selectedWiki || !searchQuery.trim()) {
      searchResults = [];
      return;
    }
    searching = true;
    try {
      searchResults = await wikiApi.search(selectedWiki.id, searchQuery);
    } catch (e) {
      console.error(e);
    }
    searching = false;
  ...
  async function ragSearch() {
    if (!selectedWiki || !ragQuery.trim()) {
      ragHits = [];
      return;
    }
    try {
      ragHits = await ragApi.search(selectedWiki.id, ragQuery, 5);
    } catch (e) {
      console.error(e);
    }
  }
  ...
  function opLabel(op: WikiOp): string {
    switch (op.op) {
      ...
      case 'delete_page':
        return `删除 ${op.path}`;
      case 'update_index':
        return `追加索引 ${(op.entries ?? []).length} 条`;
      case 'noop':
        return `跳过：${op.reason ?? ''}`;
      default:
        return op.op;
          ...
          <div class="emb-body">
            ...
            {#if embMode === 'api'}
              ...
              <div class="emb-row">
                ...
                <input
                  id="emb-model"
                  placeholder="如 text-embedding-3-small / nomic-embed-text"
                  bind:value={embModel}
                  aria-label="嵌入模型"
                />
              </div>
            ...
            <div class="emb-row">
              ...
              <label class="switch-label" for="emb-rerank">
                <input
                  id="emb-rerank"
                  type="checkbox"
                  bind:checked={rerankEnabled}
                  onchange={toggleRerank}
                />
    ...
    <div class="detail">
      ...
      <aside class="tree-pane">
        ...
        <div class="pane-header">
          ...
          <button
            ...
            title="刷新"
            aria-label="刷新页面列表"
            onclick={() => selectWiki(selectedWiki!)}
          >
            <svg
              width="13"
              height="13"
      ...
      <main class="content-pane">
        {#if selectedPath}
          ...
          {#if editing}
            <textarea class="editor" bind:value={editContent} rows="20" aria-label="编辑页面内容"
            ></textarea>
          {:else}
      ...
      <div class="query-bar">
        <div
          ...
          onkeydown={(e) => {
            ...
            if (dir) {
              e.preventDefault();
              queryMode = keys[(idx + dir + 3) % 3];
            }
          }}
        >
          <button
            class="query-tab"
            class:active={queryMode === 'ai'}
            onclick={() => (queryMode = 'ai')}
            role="tab"
            aria-selected={queryMode === 'ai'}
            tabindex={queryMode === 'ai' ? 0 : -1}>AI 写入</button
          >
          <button
            class="query-tab"
            class:active={queryMode === 'rag'}
            onclick={() => (queryMode = 'rag')}
            role="tab"
            aria-selected={queryMode === 'rag'}
            tabindex={queryMode === 'rag' ? 0 : -1}>RAG 检索</button
          >
          <button
            class="query-tab"
            class:active={queryMode === 'docs'}
            onclick={() => (queryMode = 'docs')}
            role="tab"
            aria-selected={queryMode === 'docs'}
            tabindex={queryMode === 'docs' ? 0 : -1}>文档管理</button
          >
        </div>
          ...
          <div class="query-body">
            ...
            {#if aiPlan}
              <div class="plan-preview">
                <h4>操作计划（确认后执行）</h4>
                {#each aiPlan.operations as op, i (i)}
          ...
          <div class="query-body">
            <div class="rag-row">
              <input
                placeholder="问知识库..."
                bind:value={ragQuery}
                onkeydown={(e) => e.key === 'Enter' && ragSearch()}
                aria-label="问知识库"
              />
            ...
            {#if ragHits.length > 0}
              <div class="rag-hits">
                {#each ragHits as hit (hit.chunk_id)}
                  <div class="rag-hit">
