<script lang="ts">
  import { composeStore, type ComposeStage } from '$lib/stores/compose.svelte';
...
{:else}
  <div class="compose-panel">
    ...
    {#if composeStore.totalTaskCount() > 0}
      <div class="progress-section">
        ...
        <div class="progress-bar-track">
          <div class="progress-bar-fill" style:width="{composeStore.progressPercent()}%"></div>
        </div>
      </div>
    {/if}
    ...
    {#if composeStore.session?.spec}
      <div class="section">
        <button class="section-header" onclick={() => (specExpanded = !specExpanded)}>
          ...
          {#each composeStore.session.tasks as task (task.id)}
            <div class="task-item">
              <div class="task-header">
                <span class="task-status-dot" style:background={taskStatusColor(task.status)}
                ></span>
    ...
    {#if composeStore.session?.review}
      <div class="section">
        <button class="section-header" onclick={() => (reviewExpanded = !reviewExpanded)}>
          ...
          <div class="review-content">
            {#if composeStore.session.review.critical.length > 0}
              <div class="review-group">
                ...
                {#each composeStore.session.review.critical as issue, i (i)}
            ...
            {#if composeStore.session.review.important.length > 0}
              <div class="review-group">
                ...
                {#each composeStore.session.review.important as issue, i (i)}
                  <div class="review-item">{issue}</div>
                {/each}
              </div>
            {/if}
            {#if composeStore.session.review.minor.length > 0}
              <div class="review-group">
                ...
                {#each composeStore.session.review.minor as issue, i (i)}
                  <div class="review-item">{issue}</div>
                {/each}
              </div>
            {/if}
    ...
    {#if composeStore.active}
      <div class="actions">
        ...
        <button class="action-btn stop" onclick={handleStop}> 停止 </button>
      </div>
    {/if}
  </div>
{/if}
