...
<div class="lsp-panel">
	<div class="panel-header">
		...
		<button class="refresh-btn" onclick={() => void redetect()} disabled={detecting || !workdir} title="重新检测">
				...
				<div class="server-item">
					<div class="server-header">
						...
						{#if server.status === 'running'}
							<button class="btn-stop" onclick={() => void stopServer(server.id)} disabled={busyId !== null}>
								{busyId === server.id ? '停止中…' : '停止'}
							</button>
						{:else}
							<button class="btn-start" onclick={() => void startServer(server.id)} disabled={busyId !== null}>
