...
<div class="glass-container">
  ...
  <div class="board">
    {#each columns as col (col.key)}
      <div
        class="column"
        style="background: {col.bgColor}; border: 1px solid {col.borderColor}; border-radius: var(--radius-md); padding: 12px;"
      >
        <div class="column-header">
          <span class="col-dot" style="background: {col.dotVar}"></span>
          <span class="col-title">{col.title}</span>
          <span class="col-count" style="color: {col.dotVar}; background: {col.bgColor};"
            >{cards(col.key).length}</span
          >
        </div>
            ...
            <div class="card">
              <div class="card-top">
                ...
                {#if onDeleteAgent}
                  <button
                    ...
                    onclick={(e) => {
                      e.stopPropagation();
                      if (confirm(`确定删除 Agent「${card.agent_name}」？此操作不可撤销。`)) {
                        onDeleteAgent(card.agent_id);
                      }
                    }}
                  >
                    <svg
              ...
              {#if tc.total > 0}
                <div class="task-summary">
                  <span class="task-badge todo-badge" class:visible={tc.todo > 0}
                    >{tc.todo} 待处理</span
                  >
                  <span class="task-badge doing-badge" class:visible={tc.doing > 0}
                    >{tc.doing} 进行中</span
                  >
                  <span class="task-badge done-badge" class:visible={tc.done > 0}
                    >{tc.done} 已完成</span
                  >
                </div>
              {/if}
