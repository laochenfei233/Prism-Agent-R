<script lang="ts">
  ...
  const maxTokens = $derived(data.length > 0 ? Math.max(...data.map((d) => d.tokens), 1) : 1);
  ...
  {:else}
    <svg viewBox="0 0 {width} {height}" class="chart-svg">
      ...
      {#each ticks() as tick, i (i)}
        ...
        />
        <text x={padding.left - 8} y={tick.y + 4} text-anchor="end" class="tick-label"
          >{tick.label}</text
        >
      {/each}
      ...
      {#each points as p, i (i)}
        {#if i % Math.max(1, Math.floor(points.length / 7)) === 0 || i === points.length - 1}
          <text x={p.x} y={height - 8} text-anchor="middle" class="tick-label"
