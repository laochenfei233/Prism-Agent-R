...
{#if ttsState.supported && ttsState.queue.length > 0}
	<div class="speaker" class:collapsed={collapsed}>
		{#if !collapsed}
			<div class="info">
			...
			<div class="controls">
				...
				<label class="rate" title="语速">
					...
					<input type="range" min="0.5" max="2" step="0.1" value={ttsState.rate} oninput={(e) => ttsSetRate(Number((e.target as HTMLInputElement).value))} />
				</label>
			</div>
			<button type="button" class="collapse" title="收起" onclick={() => collapsed = true}>—</button>
		{:else}
			<button type="button" class="expand" title="展开播报控制" onclick={() => collapsed = false}>
				<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3z"/><path d="M16 8c1 1.3 1.5 2.5 1.5 4s-.5 2.7-1.5 4"/></svg>
			</button>
		{/if}
	</div>
{/if}
