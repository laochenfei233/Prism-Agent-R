	...
	{:else}
		<div class="list">
			{#each meetings as meeting (meeting.id)}
				<div class="meeting-item" role="button" tabindex="0" onclick={() => onselect?.(meeting)} onkeydown={(e) => e.key === 'Enter' && onselect?.(meeting)}>
					<div class="item-main">
						<div class="item-header">
							<h3 class="item-title">{meeting.title}</h3>
							<button class="delete-btn" onclick={(e) => { e.stopPropagation(); ondelete?.(meeting); }} title="删除">
...
<style>
	...
	.delete-btn:hover { background: var(--color-bg-tertiary); color: var(--color-red); }
