...
<div class="search-settings">
	...
	<div class="actions">
		...
		<button class="btn btn-outline" onclick={testConnection} disabled={testing || provider === 'noop'}>
			{testing ? '测试中...' : '测试连接'}
		</button>
	</div>
	...
	{#if testResult}
		<div class="test-result" class:success={testResult.success} class:error={!testResult.success}>
			<div class="test-header">
				<span class="badge" class:badge-success={testResult.success} class:badge-error={!testResult.success}>
...
<style>
	...
	.label { font-size: 0.8125rem; font-weight: 500; color: var(--color-fg-secondary); }
	.hint { font-size: 0.75rem; color: var(--color-green); }
	...
	.test-result { margin-top: 1rem; padding: 0.75rem; border-radius: var(--radius-sm); border: 1px solid var(--color-border); }
	.test-result.success { border-color: var(--color-green); background: rgba(0, 200, 0, 0.05); }
	.test-result.error { border-color: var(--color-red); background: rgba(200, 0, 0, 0.05); }
	...
	.provider { font-size: 0.8125rem; color: var(--color-fg-secondary); }
	.latency { font-size: 0.75rem; color: var(--color-fg-tertiary); margin-left: auto; }
	.result-title { font-size: 0.8125rem; margin: 0.5rem 0 0; }
	.result-error { font-size: 0.8125rem; color: var(--color-red); margin: 0.5rem 0 0; }
	.footer-note { font-size: 0.75rem; color: var(--color-fg-tertiary); margin-top: 1rem; }
</style>