	...
	{:else}
		<div class="grid">
			{#each wikis as wiki (wiki.id)}
				<div class="card" role="button" tabindex="0" onclick={() => onselect?.(wiki)} onkeydown={(e) => e.key === 'Enter' && onselect?.(wiki)}>
					<div class="card-header">
						<div class="card-icon">📚</div>
						<button class="delete-btn" onclick={(e) => { e.stopPropagation(); ondelete?.(wiki); }} title="删除">
