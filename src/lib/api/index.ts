...
export const sessionApi = {
  ...
  rename: (id: string, title: string) => invoke<SessionDto>('session_rename', { id, title }),
...
export const memoryApi = {
  ...
  write: (path: string, content: string) => invoke<void>('memory_write', { path, content }),
...
export const fileApi = {
  ...
  parse: (path: string) => invoke<unknown>('file_parse', { path }),
};
...
export const settingsApi = {
  ...
  set: (key: string, value: unknown) => invoke<SettingSpecDto>('settings_set', { key, value }),
};
...
export const wikiApi = {
  ...
  readPage: (wikiId: string, path: string) => invoke<string>('wiki_read_page', { wikiId, path }),
...
export const ragApi = {
  ...
  listDocuments: (wikiId: string) => invoke<RagDocumentDto[]>('rag_list_documents', { wikiId }),
  deleteDocument: (docId: string) => invoke<void>('rag_delete_document', { docId }),
  ...
  embeddingStatus: () => invoke<EmbeddingStatusDto>('rag_embedding_status'),
  contextualConfig: (enabled: boolean) =>
    invoke<{ enabled: boolean }>('rag_contextual_config', { enabled }),
  contextualStatus: () => invoke<{ enabled: boolean }>('rag_contextual_status'),
...
export const meetingApi = {
  ...
  getTranscript: (id: string) => invoke<TranscriptSegmentDto[]>('meeting_get_transcript', { id }),
  summary: (id: string) => invoke<string>('meeting_summary', { id }),
  clean: (id: string) => invoke<string>('meeting_clean', { id }),
  qa: (id: string, question: string) => invoke<string>('meeting_qa', { id, question }),
...
export const asrApi = {
  ...
  saveConfig: (config: AsrConfigInputDto) => invoke<AsrConfigDto>('asr_save_config', { config }),
  ...
  stopRecording: (id: string) => invoke<{ transcript: string }>('meeting_stop_recording', { id }),
  pauseRecording: (id: string) => invoke<void>('meeting_pause_recording', { id }),
  resumeRecording: (id: string) => invoke<void>('meeting_resume_recording', { id }),
  cancelRecording: (id: string) => invoke<void>('meeting_cancel_recording', { id }),
};
...
export const translateApi = {
  ...
  history: (query?: string, limit?: number, offset?: number) =>
    invoke<{ items: TranslateHistoryDto[]; total: number }>('translate_history', {
      query,
      limit,
      offset,
    }),
  ...
  modelStatus: () => invoke<{ model_id: string | null }>('translate_model_status'),
};

export const glossaryApi = {
  list: (langPair?: string) => invoke<GlossaryTermDto[]>('glossary_list', { langPair }),
  add: (term: Omit<GlossaryTermDto, 'id' | 'enabled'>) => invoke<void>('glossary_add', { term }),
...
export const ocrApi = {
  ...
  providers: () => invoke<{ name: string; kind: string; available: boolean }[]>('ocr_providers'),
};
...
export const composeApi = {
  ...
  pause: (sessionId: string) => invoke<void>('compose_pause', { sessionId }),
  resume: (sessionId: string) => invoke<void>('compose_resume', { sessionId }),
  stop: (sessionId: string) => invoke<void>('compose_stop', { sessionId }),
  get: (sessionId: string) => invoke<unknown>('compose_get', { sessionId }),
};
...
export const projectIndexApi = {
  ...
  toggle: (enabled: boolean) => invoke<ProjectIndexStatusDto>('project_index_toggle', { enabled }),
...
export type SessionLifecycle =
  'Created' | 'Init' | 'Ready' | 'Running' | 'Paused' | 'Verifying' | 'Done' | 'InitFailed';
...
export const sessionLifecycleApi = {
  init: (sessionId: string) => invoke<SessionInitReport>('session_init', { sessionId }),
  state: (sessionId: string) => invoke<SessionLifecycle>('session_state_query', { sessionId }),
  cleanup: (sessionId: string) => invoke<void>('session_cleanup', { sessionId }),
