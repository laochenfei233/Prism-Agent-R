...
#### 9.5.2 对话界面详细设计（参考 Cherry Studio）

> 参考来源：Cherry Studio 桌面客户端对话界面。

**整体布局**：

```
┌──────────────────────────────────────────────────────────────────────────────┐
│  🔍 搜索助手       + 添加助手     ║  标签页 1    标签页 2    +              │
├──────────┬───────────────────────────────────────────────────────────────────┤
│          │  🌟 默认助手     mimo-v2.5 ▾               ⚙ 设置              │
│  🌟默认助手│  ─────────────────────────────────────────────────────────── │
│  📝论文写手│                                              │
...
| 交互 | 行为 |
|------|------|
| 点击 Agent | 切换当前会话（如无会话则 Draft 模式） |
| 右键 Agent | 菜单：编辑 / 复制 / 删除 / 绑定工作目录 |
| `···` 按钮 | 同右键菜单 |
| `✎` 按钮 | 进入 Agent 编辑页 |
| `+ 添加助手` | 弹出 Agent 创建向导 |
| 拖拽排序 | 按 `order_key` 持久化 |
...
- 每个标签 = 一个 `session_id`，多标签支持同时查看多个会话
- 标签显示 Agent 名称（截断 10 字符）+ 置顶星标
- `+` 新建标签 = 新建会话（直接创建或 Draft 模式）
- 标签可关闭（关闭 = 归档/保留后端会话）
- 拖拽排序、中间键关闭（复用浏览器习惯）
...
│                                         │
│ 1. 使用命令切换                          │
│    • /role default - 切换到默认角色      │
│    • /switch default - 切换到默认模式    │
└─────────────────────────────────────────┘
```

> 注：气泡中的 `/role`、`/switch` 为参考截图示例命令，Prism 的斜杠命令体系见 §9.8.1 命令面板（commands.json 自定义）——实现时以自定义命令为准。

| 元素 | 样式 |
|------|------|
| 用户气泡 | 右对齐、浅色背景（var(--color-fill)）、圆角 16px |
| AI 气泡 | 左对齐、白色/深色背景（var(--color-card)）、圆角 16px |
| Agent 头像 | emoji 或图片，24×24px |
| 处理时间 | 次要文字色（var(--color-foreground-secondary)），气泡下方 |
| Markdown | 支持标题/列表/代码块/表格渲染 |
...
- 点击模型名 → 下拉列表（当前可用模型 + 按 Provider 分组）
- 模型图标 + 名称 + 能力徽标（视觉/推理/编码/搜索/工具）
- 会话内切换模型（不影响已发送消息）

### 9.5.3 设置界面设计（参考 Cherry Studio）

> 参考来源：Cherry Studio 设置界面。
...
│ 设置导航         │ 中间列表/内容     │ 右侧详情                       │
│                  │                  │                                │
│ ☁ 模型服务       │ 🔍 搜索模型平台   │  Xiaomi MiMo    ⚙  [ON]       │
│ ☑ 默认模型       │ ──────────────── │                                │
│ 📦 本地模型       │ 🔵 二狗子     ●  │  API 密钥  获取密钥             │
│ 📡 API 网关      │ 🟢 Xiaomi MiMo  ●│  ●●●●●●●●●●●●●●●●●●         │
│                  │ 🟢 Ollama     ●  │                                │
│ ── 工具 ──      │ 🟢 llama.cpp  ●  │  API 地址  添加端点             │
│ 🔧 MCP          │ 🟢 CherryIN   ●  │  https://api.xiaomimimo.com    │
│ 📋 技能          │ 🟢 Gemini     ●  │                                │
│ 🔍 网络搜索      │ 🟢 DMXAPI     ●  │  ── 模型 ──                    │
│ 📄 文档处理      │ 🟢 硅基流动    ●  │  模型 📋 🔍 📊  获取模型列表 + │
│ 🖼 OCR          │ 🔵 O3         ●  │  ┌────────────────────────────┐ │
...
| 分组 | 项目 | 功能 |
|------|------|------|
| （顶） | 模型服务 | Provider/API Key 管理 |
| | 默认模型 | 全局默认模型选择 |
| | 本地模型 | Ollama/llama.cpp 本地部署 |
| | API 网关 | 统一网关配置 |
| 工具 | MCP / 技能 / 网络搜索 / 文档处理 / OCR | 各工具开关与配置 |
| 偏好 | 外观 / 通知 / 数据 / 用量统计 | 个性化设置 |
| 效率 | 定时任务 / 快捷键 / 快捷指令 | 效率工具 |
| 系统 | 系统 / 环境依赖 | 底层配置 |

> 注：参考截图中 Cherry Studio 的「频道 / 快捷助手 / 划词助手」为客户端特有功能，Prism Agent R 未规划（见 [S3]/[S5]），此处不列入；「定时任务 / 快捷键 / 快捷指令」对应 Prism 的 §9.8 快捷键体系与 §9.8.1 命令面板。

**Provider 列表**（中间栏）：

| 元素 | 样式/交互 |
|------|----------|
| Provider 图标 | 32×32px，品牌色/Logo |
| Provider 名称 | 16px，选中态加粗 |
| 状态指示点 | 绿色=可用 / 灰色=未配置 / 红色=错误 |
| 选中态 | 背景色 + 圆角 + 右侧 ··· 菜单 |
| 搜索 | 顶部搜索框，实时过滤 |
| `+ 添加服务商` | 底部固定，弹出服务商选择弹窗 |
...
> 注：能力徽标在 Provider 侧用 `models.kind` 字段映射（chat/embedding/vision/asr，见 §5.2），前端按 kind 渲染对应徽标；徽标图例为本设计建议，实现时可用 CSS 图标替代 emoji。
