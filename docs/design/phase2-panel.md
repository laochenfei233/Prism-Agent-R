...
| 阈值 | 触发 | 反馈 |
|------|------|------|
| 上下文 80% | 会话内上下文占用超 80% | 进度条变橙 + 侧边栏徽标「建议新会话」 |
| 上下文 95% | 超 95% | 变红 + 系统通知（若启用） |
| 日费用预算 80% | `usage:stats` 今日费用超预算 80% | toast + 用量 Tab 顶栏警示条 |
| 日费用预算 100% | 超预算 | 可选：自动阻止新请求（需用户开启「费用硬上限」） |

- **配置**：设置页 → 用量 → 预算阈值（日费用上限、预警百分比）、通知开关
- **存储**：`preferences` 键 `usage.alert.*`；费用预算按 provider/model 单价表（§9.9 已有）
- **通知通道**：应用内 toast + 系统通知（`tauri-plugin-notification`，可选）；预警不打断对话（非模态）
- **实现**：`usage:updated` 事件在消息完成后推送 → 前端 `UsageAlertStore` 判断阈值 → 触发反馈；费用硬上限在后端 ChatService 发送前检查（`usage:stats` 实时查询）

**可能错误 + 处理方法**：

| 错误 | 检测 | 处理 | 反馈 |
|------|------|------|------|
| 用量统计缺失（旧消息无 usage） | usage 字段为 null | 按 0 计 + 标记「估算」 | 「部分用量为估算值」 |
| 单价表缺失（新模型） | 单价查找失败 | 费用按 0 计 + 标记「未定价」 | 「模型 X 未定价，费用暂估 0」 |
| 阈值配置非法（预算为负） | 配置校验 | 回退默认阈值 | 「预算配置无效，已用默认」 |
| 费用硬上限误拦截 | 发送前检查误判 | 允许「临时放行一次」（需确认） | 「已达费用上限，临时放行？」 |
| 通知被系统禁用 | 通知权限检查 | 降级为应用内 toast | 无感（自动降级） |
...
### 10.4 Skill 市场搜索（Phase 2，自 phase1 §10.4 移入）
